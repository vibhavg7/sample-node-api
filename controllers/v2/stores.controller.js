...
exports.fetchStoreCategories = function(req, res, next) {
...
exports.updatestoreCategoryStatus = function(req, res, next) {
    const updatestoreCategory = {};
    // updatedStoreProduct.store_cost_price = '';
    updatestoreCategory.status = +req.body.status;

    pool.getConnection(function (err, dbConn) {
        dbConn.query("UPDATE store_category_mapping SET ? WHERE store_category_mapping_id = ?",
            [updatestoreCategory, req.params.storeCategoryMappingId], function (err, updatedStoreProduct) {
                if (err) {
                    console.log("error: ", err);
                    res.json({
                        "message": "error updating store category",
                        "status": 400,
                    });
                }
                else {
                    res.json({
                        ...
                        "status": 200,
                        "store_category_mapping_id": req.params.storeCategoryMappingId
                    });
                }
                dbConn.release();
            });
...
exports.fetchStoreSubCategoriesInfoById = function (req, res) {
    let storeCategoryStatus = req.body.status;
    console.log(storeCategoryStatus);
    let sql = `CALL GET_STORE_CATEGORIES_UPDATED(?,?)`;
    pool.getConnection(function (err, dbConn) {
        dbConn.query(sql,
            [req.params.storeId, storeCategoryStatus], function (err, storeCategory) {
                if (err) {
                    console.log("error: ", err);
                }
                else {
